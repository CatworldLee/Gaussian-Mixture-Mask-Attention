...
class Attention(nn.Module):
    def __init__(self, dim, num_patches, heads = 8, dim_head = 32, dropout = 0., is_GMM=False ,is_SLM=False, num_kernals=5, mask=None):
        super().__init__()
        ...
        if is_GMM:
            ...
            self.mask = Simple_Mask(num_heads=self.heads, num_patches=self.num_patches - 1)
        else:
            self.mask = None
    ...
    def forward(self, x):
        ...
        if self.mask is not None:
            dots[:, :, 1:, 1:] = self.mask(dots[:, :, 1:, 1:].clone())
...
class Transformer(nn.Module):
    def __init__(self, dim, num_patches, depth, heads, dim_head, mlp_dim_ratio, dropout = 0., stochastic_depth=0.,
                 is_GMM=False ,is_SLM=False, num_kernals=5,):
        super().__init__()
        self.layers = nn.ModuleList([])
        num_patches = num_patches**2 + 1

        self.mask = nn.Parameter(On_attention_gaussian_mask(num_patches - 1), requires_grad=False)

        for i in range(depth):
            self.layers.append(nn.ModuleList([
                PreNorm(num_patches, dim, Attention(dim, num_patches, heads = heads, dim_head = dim_head, dropout = dropout,
                                                    is_GMM=is_GMM ,is_SLM=is_SLM, num_kernals=num_kernals, mask=self.mask)),
...
class PiT(nn.Module):
    def __init__(self, *, img_size, patch_size, num_classes, dim, depth, heads, mlp_dim_ratio, dim_head = 64, dropout = 0., 
                 emb_dropout = 0., stochastic_depth=0., is_GMM=False ,is_SLM=False, num_kernals=5,
                 ):
        ...
        for ind, (layer_depth, layer_heads) in enumerate(zip(depth, heads)):
            ...
            self.layers.append(Transformer(dim, output_size, layer_depth, layer_heads,
                                           ...
                                           is_GMM=is_GMM ,is_SLM=is_SLM, num_kernals=num_kernals,))
